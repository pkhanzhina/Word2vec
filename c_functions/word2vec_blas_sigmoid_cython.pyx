...
cdef REAL_t[EXP_TABLE_SIZE] expTable
...
cdef init():
    for i in range(EXP_TABLE_SIZE):
        expTable[i] = <REAL_t>exp((i / <REAL_t>EXP_TABLE_SIZE * 2 - 1) * MAX_EXP)
        expTable[i] = <REAL_t>(expTable[i] / (expTable[i] + 1))
    return 0
...
cdef inline void fast_sentence(
    ...
    for i in range(codelen):
        ...
        f = <REAL_t>sdot(&size, &w0[row_w0], &one, &w1[row_w1], &one)
        ...
        saxpy(&size, &g, &w1[row_w1], &one, work, &one)
        saxpy(&size, &g, &w0[row_w0], &one, &w1[row_w1], &one)

    saxpy(&size, &ONEF, work, &one, &w0[row_w0], &one)
